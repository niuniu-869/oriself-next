...
>
> v2.4.2 修订：基于首轮 160 场 benchmark 的深挖（findings_v1.md / analysis.md），针对 tired / literary 风格的 E/I 翻 I、converge JSON 引号崩、对话后半段"长陈述盖住问号"三个真实问题打的一组互相协同的 patch。
>
> v2.4.3 补丁：基于 480 场扩大 benchmark（16×30）暴露的两个结构性短板——"思考型被试（ENTJ/ENFJ）的 E 信号被持续深挖的认知层吃掉"、"literary 被试让 LLM 滑入共创诗意"——补两处最小改动（灵魂 §1 加"不做诗人"限定；phase2-3 + phase4 加"思考型→社交场景题 pivot"）。到此停手。
